...
static inline int64_t sys_open(const char* pathname, int flags, int mode) {
    return syscall3(2, (uint64_t)pathname, flags, mode);
}
static inline int64_t sys_close(int fd) {
    return syscall1(3, (uint64_t)fd);
}
...
static inline int64_t sys_ioctl(int fd, uint64_t req, uint64_t arg) {
    return syscall3(16, (uint64_t)fd, req, arg);
}
...
void test_framebuffer() {
    ...
    int fb = sys_open("/dev/fb0", 0x0003, 0);
    if (fb < 0) {
    ...
    struct fb_info info;
    int64_t ret = sys_ioctl(fb, 0x4601, (uint64_t)&info);
    if (ret != 0) {
...
void test_kbio_events() {
    ...
    int kbio = sys_open("/dev/kbio", 0x0001, 0);
    if (kbio < 0) {
...
void test_mouse_events() {
    ...
    int mfd = sys_open("/dev/mouse", 0x0001, 0);
    if (mfd < 0) {
...
void test_mmap_anonymous() {
    print_str("\n--- TEST: anonymous mmap ---\n");
    void *p = sys_mmap(0, 8192, 0x1 | 0x2, 0x02 | 0x20, -1, 0);
    if (p == ((void *)-1)) { print_str("  [FAIL] mmap returned MAP_FAILED\n"); return; }
...
void test_mmap_mprotect() {
    print_str("\n--- TEST: mprotect ---\n");
    void *p = sys_mmap(0, 4096, 0x1 | 0x2, 0x02 | 0x20, -1, 0);
    if (p == ((void *)-1)) { print_str("  [FAIL] mmap failed\n"); return; }
    ((uint8_t *)p)[0] = 0xBE;
    if (sys_mprotect(p, 4096, 0x1) != 0) { print_str("  [FAIL] mprotect PROT_READ\n"); sys_munmap(p, 4096); return; }
    print_str("  OK: mprotect PROT_READ applied\n");
    if (((uint8_t *)p)[0] != 0xBE) { print_str("  [FAIL] not readable\n"); sys_munmap(p, 4096); return; }
    print_str("  OK: data still readable\n");
    if (sys_mprotect(p, 4096, 0x1 | 0x2) != 0) { print_str("  [FAIL] restore\n"); sys_munmap(p, 4096); return; }
    ((uint8_t *)p)[0] = 0xEF;
...
void test_mmap_multi() {
    ...
    void *a = sys_mmap(0, 4096, 0x1 | 0x2, 0x02 | 0x20, -1, 0);
    void *b = sys_mmap(0, 8192, 0x1 | 0x2, 0x02 | 0x20, -1, 0);
    void *c = sys_mmap(0, 16384, 0x1, 0x02 | 0x20, -1, 0);
    if (a == ((void *)-1) || b == ((void *)-1) || c == ((void *)-1)) {
        print_str("  [FAIL] one mapping failed\n");
        if (a != ((void *)-1)) sys_munmap(a, 4096);
        if (b != ((void *)-1)) sys_munmap(b, 8192);
        if (c != ((void *)-1)) sys_munmap(c, 16384);
        return;
    }
